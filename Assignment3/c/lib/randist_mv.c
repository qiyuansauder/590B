...
int wu_ran_mv_normal(const gsl_rng *r, const gsl_vector *mu,
...
int wu_ran_mv_lognormal(const gsl_rng *r, const gsl_vector *mu,
			const gsl_matrix *Sigma, gsl_vector *x)
{
    const int k = mu->size;
    int i;
...
int wu_ran_mv_t(const gsl_rng *r, const gsl_vector *mu,
...
int wu_ran_wishart(const gsl_rng *r, const double nu,
...
int wu_ran_invwishart(const gsl_rng *r, const double nu,
		      const gsl_matrix *V, gsl_matrix *X)
{
  const int k = V->size1;
...
int wu_ran_dirichlet(const gsl_rng *r, const gsl_vector *alpha,
		  gsl_vector *sample)
{
    ...
    gsl_vector_scale(sample, 1.0/sum);
...
double wu_ran_mv_normal_pdf(const gsl_vector *x, const gsl_vector *mu,
			    const gsl_matrix *Sigma)
{
  ...
  den = -0.5*den - k/2.0 * log(2.0*M_PI) - 0.5*log(det);
  den = exp(den);
...
double wu_ran_mv_normal_pdf_log(const gsl_vector *x, const gsl_vector *mu,
				const gsl_matrix *Sigma)
{
  const int k = x->size;
  int s;
...
double wu_ran_mv_t_pdf(const gsl_vector *x, const gsl_vector *mu,
		    const gsl_matrix *Sigma, const double nu)
{
  ...
  temp = pow((1+temp), (nu+ (double) k)/2.0 );
  temp *= gsl_sf_gamma(nu/2.0) * pow(nu, k/2.0) * pow(M_PI, k/2.0) * sqrt(det);

  den = gsl_sf_gamma((nu+ (double) k)/2.0);
  den /= temp;
...
double wu_ran_wishart_pdf(const gsl_matrix *X, const double nu, const gsl_matrix *V)
{
  const int k = X->size1;
  ...
  temp = 0.5*(nu-k-1)*log(detX) - 0.5*nu*k*log(2.0) -0.5*nu*log(detV);
  temp -= wu_sf_mv_lngamma(nu/2.0, k);
  den += temp;
...
double wu_ran_invwishart_pdf(const gsl_matrix *X, const double nu, const gsl_matrix *V)
{
  const int k = X->size1;
  double detX, detV, den, temp;
  int s, i;
  ...
  gsl_linalg_LU_decomp(work_k_k, p, &s);
  detX = gsl_linalg_LU_det(work_k_k, s);
  gsl_linalg_LU_invert(work_k_k, p, Xinv);
  ...
  gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, V, Xinv, 0.0, work_k_k);
  den = 0;
  for (i=0; i<k; i++)
    {
      den -= 0.5 * gsl_matrix_get(work_k_k, i, i);
...
double wu_sf_mv_gamma(const double x, const int p)
{
  double v;
  int i;

  v = pow(M_PI, p*(p-1)/4.0);

  for (i=0; i<p; i++)
    {
      v *= gsl_sf_gamma(x-i/2.0);
    }
...
double wu_sf_mv_lngamma(const double x, const int p)
{
  double v;
  int i;

  v = p*(p-1)*log(M_PI)/4.0;

  for (i=0; i<p; i++)
    {
      v += gsl_sf_lngamma(x-i/2.0);
    }
